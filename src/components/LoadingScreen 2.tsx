...
export default function LoadingScreen({ onComplete, screenSrc = "/your-custom-screen.jpg" }: LoadingScreenProps) {
  ...
  const applyTextBounds = useCallback(() => {
    const img = imgRef.current;
    const text = textRef.current;
    if (!img || !text) return;

    const r = img.getBoundingClientRect();
    const sl = r.width * SCREEN.left;
    const st = r.height * SCREEN.top;
    const sw = r.width * SCREEN.width;
    const sh = r.height * SCREEN.height;
  ...
  // Dynamically position and size your custom image inside the TV glass boundary
  const positionScreenImage = useCallback(() => {
    const img    = imgRef.current;
    ...
    // Match the text bounds exactly to the screen bounds for perfect flexbox centering
    if (text) {
      text.style.left = `${sl}px`;
      text.style.top = `${st}px`;
      text.style.width = `${sw}px`;
  ...
  useEffect(() => {
    const PHASE = {
      ...
      settle:    { start: 3200, end: 3900 },
      imageOn:   { start: 3700, end: 4000 }, 
      bgFade:    { start: 4000, end: 4400 },
    };
  ...
  }, [onComplete, positionScreenImage]);

  useEffect(() => {
    applyTextBounds();
    positionScreenImage();
    window.addEventListener('resize', positionScreenImage);
    window.addEventListener('resize', applyTextBounds);
    return () => window.removeEventListener('resize', positionScreenImage);
  }, [applyTextBounds, positionScreenImage]);
