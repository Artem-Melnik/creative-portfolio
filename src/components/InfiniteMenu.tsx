...
const defaultItems: MenuItem[] = [
  {
    image: '/Portfolio%20Images/Photography/AGC_20250710_192207751.webp',
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, onItemClick }) => {
  ...
  const handleButtonClick = () => {
    if (onItemClick && activeItem) {
      const index = items.findIndex(item => item === activeItem);
      onItemClick(index >= 0 ? index : 0, activeItem);
      return;
    }
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        ...
        `}
          >
            <ArrowUpRight className="select-none relative text-white text-[26px]" />
            {/* <p className="select-none relative text-[#120F17] top-[2px] text-[26px]">↗️&#x2197;</p> */}
          </div>
        </>
      )}
    </div>
